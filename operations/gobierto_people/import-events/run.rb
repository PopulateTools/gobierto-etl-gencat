...
Bundler.require
require_relative "../../../utils/events_importer"
...
# Samples:
#
#   /path/to/project/operations/gobierto_people/import-events/run.rb /tmp/gencat/downloads/datasets/events.csv gencat.gobierto.test
#
...
puts "[START] import-events/run.rb with path=#{ path_arg }, domain=#{ site.domain }"

if file.exist?
  importer = Utils::EventsImporter.new(path: file.file_path, site: site)
  puts "Importing events from file: #{ file.file_path }"
  importer.import!
  if importer.errors?
    errors_output = Utils::LocalStorage.new(path: "output/events_errors.txt", content: importer.errors.pretty_inspect)
  ...
  puts "Events file is not present. Nothing to do"
end

puts "[END] import-events/run.rb"
exit 0
